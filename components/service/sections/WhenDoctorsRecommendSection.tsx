...
export type WhenDoctorsRecommendSectionProps = SectionBaseProps & {
  /** CMS: whenRecommended.eyebrow */
  eyebrow?: string;
  /** CMS: whenRecommended.heading */
  title?: string;
  /** CMS: whenRecommended.intro */
  intro?: string;
  /** CMS: whenRecommended.items */
  items?: string[];
};

/**
 * Clinical criteria — when doctors typically recommend treatment.
 * React owns layout/chrome; CMS owns eyebrow, title, intro, and items.
 * Empty items AND no intro → null.
 */
export function WhenDoctorsRecommendSection({
  id = "when-recommended",
  eyebrow,
  title,
  intro,
  items,
  className,
}: WhenDoctorsRecommendSectionProps) {
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(700px_160px_at_50%_0%,rgba(21,87,160,0.08),transparent_55%)]"
        aria-hidden
      />
          ...
          <ol className="grid list-none gap-3 sm:grid-cols-2 sm:gap-4">
            {list.map((item, i) => (
              <li
                ...
                className={cn(
                  "relative flex gap-3.5 overflow-hidden rounded-2xl border border-slate-200/90 bg-white p-4 sm:p-5",
              ...
              >
                <span
                  className="absolute inset-y-0 left-0 w-1 bg-[#1557A0]"
                  aria-hidden
                />
                <span
                  className={cn(
                ...
                <span className="min-w-0 pt-1.5 text-sm leading-relaxed text-slate-700 sm:text-[0.9375rem]">
