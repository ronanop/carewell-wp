...
export type ResultsExpectationsSectionProps = SectionBaseProps & {
  /** CMS: expectations.eyebrow */
  eyebrow?: string;
  /** CMS: expectations.heading */
  title?: string;
  /** CMS: expectations.intro */
  intro?: string;
  /** CMS: expectations.items */
  items?: string[];
};

/**
 * Results & realistic expectations checklist.
 * React owns layout/chrome; CMS owns eyebrow, title, intro, and items.
 * Empty items AND no intro → null.
 */
export function ResultsExpectationsSection({
  id = "expectations",
  eyebrow,
  title,
  intro,
  items,
  className,
}: ResultsExpectationsSectionProps) {
