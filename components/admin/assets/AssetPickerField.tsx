...
import { refreshAssetRefAction } from "@/lib/assets/actions/assetActions";
import { toAssetRef } from "@/lib/assets/domain";
...
/**
 * Universal asset field — stores MediaRef (WordPress mediaId + snapshot).
 * Opens Asset Manager instead of WordPress Admin / legacy MediaPicker.
 */
export function AssetPickerField({
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      const result = await refreshAssetRefAction(value);
      if (!result.ok) return;
...
/** @deprecated Prefer AssetPickerField */
export const MediaPickerField = AssetPickerField;
