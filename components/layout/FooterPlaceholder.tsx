...
const socialLinks = [
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className="size-6 sm:size-4 lg:size-5">
...
const bodyClass =
  "text-[0.78rem] font-medium leading-[1.5] text-slate-700 sm:text-[0.9375rem] sm:font-normal sm:text-muted-foreground lg:text-[1rem] lg:leading-normal";

const aboutBodyClass = `${bodyClass} max-[767px]:font-bold`;

const linkClass =
  "text-[0.78rem] font-medium leading-[1.5] text-slate-700 no-underline transition-colors hover:text-primary hover:no-underline sm:text-[0.9375rem] sm:font-normal sm:text-muted-foreground lg:text-[1rem] lg:leading-normal";
      ...
      <div className="container-content py-6 sm:py-10 lg:py-[var(--section-padding-y-desktop)]">
        <div className="grid grid-cols-2 gap-x-4 gap-y-4 sm:gap-x-8 sm:gap-y-6 lg:grid-cols-4 lg:gap-8">
          ...
          <div className="col-span-2 lg:col-span-1">
            ...
            <ul className="mt-3 flex items-center justify-center gap-4 max-[767px]:justify-center sm:mt-2 sm:justify-start lg:mt-5 lg:gap-4">
              {socialLinks.map((social) => (
                <li key={social.label}>
                  <a
                    href={social.href}
                    aria-label={social.label}
                    className={`inline-flex size-11 items-center justify-center transition-opacity hover:opacity-80 sm:size-8 lg:size-10 ${social.brandClass}`}
                  >
                    {social.icon}
                  </a>
                </li>
        ...
        <div className="mt-6 border-t border-border pt-4 sm:mt-8 sm:pt-6 lg:mt-12 lg:pt-8">
          <p className="text-[0.78rem] font-medium leading-[1.5] text-slate-700 sm:text-[0.875rem] sm:font-normal sm:text-muted-foreground lg:text-[1rem] lg:leading-normal">
